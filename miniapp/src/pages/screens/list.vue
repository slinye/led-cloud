    ...
    <scroll-view class="list" scroll-y @scrolltolower="loadMore" v-if="isLoggedIn">
      <view class="list-item" v-for="item in list" :key="item.id" @click="goDetail(item.id)">
        <view class="item-left">
          <view class="status-dot" :class="isOnline(item) ? 'online' : 'offline'"></view>
...
async function loadData() {
  ...
  loading.value = true
  try {
    const res = await getScreenList(page.value, 20, '', true)
    if (res.code === 200) {
...
function goDetail(id) {
  uni.navigateTo({ url: `/pages/screens/detail?id=${id}` })
}
