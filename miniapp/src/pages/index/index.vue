  ...
  <view class="page">
    ...
    <view class="section" v-if="isLoggedIn">
      ...
      <view class="quick-actions">
        ...
        <view class="action-item" @tap="navTo('/pages/schedule/list')">
          <view class="action-icon bg-teal">⏰</view>
          <text>定时播放</text>
        </view>
    ...
    <view class="section" v-if="isLoggedIn && screens.length > 0">
      ...
      <view class="screen-list">
        <view class="screen-item" v-for="item in screens" :key="item.id">
          <view class="screen-status" :class="isOnline(item) ? 'online' : 'offline'"></view>
...
async function loadData() {
  try {
    const [dashRes, screenRes] = await Promise.all([
      getDashboard(true).catch(() => ({ data: {} })),
      getScreenList(1, 5, '', true).catch(() => ({ data: { records: [] } }))
    ])
    if (dashRes.code === 200) {
...
function isOnline(item) {
  ...
  const hb = new Date(item.lastHeartbeat).getTime()
  return (now - hb) < 60000
}
...
function navTo(url) {
  if (url.startsWith('/pages/schedule')) {
    uni.navigateTo({ url })
  } else {
    uni.switchTab({ url })
  }
